...
-}
module ISA.Types.Instruction.Encode (
    -- * encode on instruction into a binary instruction code
    encode,
    concretiseInstr,
) where
...
concretiseImm (Imm i) =
    case toImm i of
        ...
        Left sym ->
            error $
                "Instruction.encode: symbolic immediate argument"
                    <> show sym
...
encode i = InstructionCode . fromBitsLEWord16 $ case i of
    ...
    Instruction (Add r addr) ->
        (asBools . opcode $ i) ++ encodeRegister r
            ++ encodeMemoryAddress addr
    Instruction (AddI r (Imm (byte))) ->
        (asBools . opcode $ i)
            <> encodeRegister r
            ++ encodeByte (Imm byte)
    Instruction (Jump (Imm (byte))) ->
        (asBools . opcode $ i)
            <> encodeByte (Imm byte)
            <> pad 2
