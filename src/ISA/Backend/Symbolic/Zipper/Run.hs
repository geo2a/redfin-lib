...
readInstructionRegister = do
    ...
    case x of
        ...
        (Left sym) ->
            error $
                "Engine.readInstructionRegister: "
...
execute todo = \case
  ...
  where
    ...
    reach ctx = do
        ...
        let constrs =
                toSMT
                    (symbolicMemory vars (dumpMemory ctx))
                    vars
                    (foldr (&&&) true $ _pathCondition ctx : map snd (_constraints ctx))
        SBV.query $ do
...
analyse i =
    let (Reads rs, Writes ws) = dependencies (instructionSemanticsS i)
     in if
                | F Halted `elem` ws -> pure Zero
                | IC `elem` ws ->
                    if
                            | F Condition `elem` rs -> branchOn Condition <$> getFocused
...
branchOn f s = case f of
    ...
    Condition ->
        case getBinding (F Condition) s of
            ...
            Just cond ->
                let yes =
                        putBinding (F Condition) true $
                            s{_pathCondition = cond &&& (_pathCondition s)}
                    no =
                        putBinding (F Condition) false $
