...
instance Show a => Show (Block a) where
    show (MkBlock a is _ t) =
        unlines $
            (map (\(a, i) -> show a <> ". " <> show i) . zip [a ..] $ is)
                ++ [ "Targets: " <> case t of
                        Zero -> "⊥"
                        One n -> show (fst n)
...
basicBlocks Nothing = map postprocessBlock . basicBlocksImpl defaultInit . assemble
  where
    defaultInit = emptyCtx{_bindings = Map.fromList [(IC, 0), (F Condition, false)]}
...
-}
basicBlocksImpl ::
    (Addressable a, Value a) =>
    Context a ->
    [(CAddress, Instruction a)] ->
    [Block a]
...
computeTargets ::
    ...
    (CAddress, Instruction a) ->
    Simulate a (ZeroOneTwo (CAddress, a))
